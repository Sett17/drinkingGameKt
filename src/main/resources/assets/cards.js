const endCard = {"title": "Das wars", "text": "Danke fürs spielen!\nWische einfach diese Karte weg.", "sips": -1, "all": true, "set": "Ehere"}
const cards = [
    {
        "title": "Schusselig?",
        "text": "Hast du schonmal dein Handy verloren?",
        "sips": 3,
        "all": false,
        "set": "Wahrheit"
    },
    {
        "title": "Schande",
        "text": "Hast du schonmal harte Drogen  zu dir genommen?",
        "sips": 3,
        "all": false,
        "set": "Wahrheit"
    },
    {
        "title": "Was muss das muss",
        "text": "Hast du schonmal in einen Blumentopf gepinkelt?",
        "sips": 2,
        "all": true,
        "set": "Wahrheit"
    },
    {
        "title": "Auf EX",
        "text": "Die Person die zuerst ihr Glas leert, bestimmt eine Person die eine Getränk ihrer Wahl trinken muss",
        "sips": -1,
        "all": true,
        "set": "Pflicht"
    },
    {
        "title": "Papagei",
        "text": "[NAME1] muss alles wiederholen, was du sagst",
        "sips": 1,
        "all": false,
        "set": "Virus"
    },
    {
        "title": "Blatt vorm Mund",
        "text": "Worte für das weibliche Genital. [NAME1] startet! Wem als erstes nix mehr einfällt trinkt",
        "sips": 2,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Längenvergleich",
        "text": "Trink für jede Person, die größer ist als  du",
        "sips": 1,
        "all": false,
        "set": "Pflicht"
    },
    {
        "title": "Wer zuerst kommt...",
        "text": "Wer als letztes angekommen ist, trinkt um seine Verspätung aufzuholen",
        "sips": 5,
        "all": false,
        "set": "Pflicht"
    },
    {
        "title": "Hoch die Hände",
        "text": "Hebt einen Arm. Wer ihn als erstes runternimmt trinkt, und wer ihn als letztes runternimmt verteilt",
        "sips": 5,
        "all": false,
        "set": "Minigame"
    },
    {
        "title": "Tauchstation",
        "text": "Halte deinen Atem für 30 Sekunden",
        "sips": 4,
        "all": false,
        "set": "Pflicht"
    },
    {
        "title": "Wer wirds sein",
        "text": "Zeigt auf den, der für Geld alles macht. Der 'Gewinner' trinkt",
        "sips": 4,
        "all": true,
        "set": "Wahrheit"
    },
    {
        "title": "Hunger?",
        "text": "Burger bei McDonalds. Wer denselben Burger wiederohlt oder wem nichts mehr einfällt, trinkt. [NAME1] beginnt",
        "sips": 3,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Auf der Suche?",
        "text": "Trinke wenn du auf Tinder unterwegs bist",
        "sips": 2,
        "all": true,
        "set": "Wahrheit"
    },
    {
        "title": "Pfui!",
        "text": "Ihr Raucher, trinkt auf das Wohlsein eurer Lungen",
        "sips": 5,
        "all": true,
        "set": "Wahrheit"
    },
    {
        "title": "Abstimmung!",
        "text": "Vanille- oder Schokoladeneis? Die Außenseiter trinken",
        "sips": 1,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Du bist dran!",
        "text": "Du darfst eine neue Regel entwerfen",
        "sips": 2,
        "all": false,
        "set": "Virus"
    },
    {
        "title": "Sorry!",
        "text": "Wir mögen dich alle sehr, aber du musst trotzdem trinken",
        "sips": 7,
        "all": false,
        "set": "Pflicht"
    },
    {
        "title": "Wer wirds sein",
        "text": "Zeigt auf den, der heute noch am ehesten kotzt. Der 'Gewinner' trinkt",
        "sips": 1,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Wirklich?",
        "text": "Hast du schonmal einen Salat bei McDonald's bestellt?",
        "sips": 2,
        "all": true,
        "set": "Wahrheit"
    },
    {
        "title": "Auf Ex",
        "text": "Die Person die als letztes aufgestanden ist und sich einmal im Kreis gedreht hat, trinkt",
        "sips": -1,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Wer wirds sein",
        "text": "Wählt die Person, die sich am leichtesten kidanppen ließe. Der 'Gewinner' trinkt",
        "sips": 3,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "So eine/r...",
        "text": "Trink, wenn du so eine/r bist, der den ganzen Film lang Fragen stellt...",
        "sips": 4,
        "all": false,
        "set": "Wahrheit"
    },
    {
        "title": "Fürs Vaterland",
        "text": "Sing den Anfang der Nationalhymne. Bei Fehlern trinken",
        "sips": 3,
        "all": false,
        "set": "Pflicht"
    },
    {
        "title": "Styler",
        "text": "Trink, wenn du eine Kappe oder Mütze trägst",
        "sips": 1,
        "all": true,
        "set": "Wahrheit"
    },
    {
        "title": "Abstimmung",
        "text": "Reich in Nordkorea oder arm in Deutschland sein? Die 'Außenseiter' trinken",
        "sips": 4,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Längenvergleich",
        "text": "Trink für jede Person die kleiner ist als du",
        "sips": 1,
        "all": false,
        "set": "Pflicht"
    },
    {
        "title": "Die Großeltern",
        "text": "Alle, die älter sind als du, trinken",
        "sips": 5,
        "all": false,
        "set": "Minigame"
    },
    {
        "title": "Best of 3",
        "text": "Spiele Schere-Stein-Papier gegen [NAME1]",
        "sips": 3,
        "all": false,
        "set": "Minigame"
    },
    {
        "title": "Verständlich",
        "text": "Trink, wenn du dich schonmal krankgemeldest hast, weil du verkatert warst.",
        "sips": 4,
        "all": true,
        "set": "Wahrheit"
    },
    {
        "title": "Blatt vorm Mund",
        "text": "Worte für das männliche Genital. [NAME1] startet! Wem als erstes nix mehr einfällt trinkt",
        "sips": 5,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Wer kennt sich aus",
        "text": "Getränke die mit 'H' anfangen, gegen einen Spieler deiner Wahl. Der Verlierer trinkt",
        "sips": 2,
        "all": false,
        "set": "Minigame"
    },
    {
        "title": "double or nothing",
        "text": "Alle ungeraden Schlucke werden verdoppel",
        "sips": 1,
        "all": true,
        "set": "Virus"
    },
    {
        "title": "Nummer 17 mit süß-sauer",
        "text": "Gerichte die man bei jedem Asiaten bekommen kann. [NAME1] startet! Wem als erstes nix mehr enifällt trinkt!",
        "sips": 3,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Gewinn-Ausgaben=PROFIT",
        "text": "Trinke wenn du etwas in Richtung Wirtschaft oder BWL studierst",
        "sips": 3,
        "all": true,
        "set": "Wahrheit"
    },
    {
        "title": "Wieviele Finger?",
        "text": "Wenn du Brillenträger bist, trinke",
        "sips": 3,
        "all": true,
        "set": "Pflicht"
    },
    {
        "title": "Wünsch dir was",
        "text": "Sag etwas was du noch nie gemacht hast, aber schon immer machen wolltest. Jeder der es schon gemacht hat trinkt! [NAME1] startet",
        "sips": 4,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "ABCDEFG",
        "text": "Sage das Alphabet rückwärts auf, wenn du einen Fehler machst fange von vorne an. Du hast 3 Leben!",
        "sips": -1,
        "all": false,
        "set": "Pflicht"
    },
    {
        "title": "Uuhhhhh",
        "text": "Trink so viele Schlücke wie es schöne Brüste im Raum gibt",
        "sips": -1,
        "all": false,
        "set": "Pflicht"
    },
    {
        "title": "Schon komisch",
        "text": "Trink wenn du beim Masturbieren schon einmal an deine/n Cousin/e gedacht hast",
        "sips": 4,
        "all": false,
        "set": "Wahrheit"
    },
    {
        "title": "Passiert halt",
        "text": "Trink wenn du schon einmal mit deinem rechten Nachbarn gekotzt hast",
        "sips": 5,
        "all": false,
        "set": "Wahrheit"
    },
    {
        "title": "Was muss das muss",
        "text": "Trinke falls du schon mal auf dem Klo mit jemandem telefonierst hast, +1 Schluck falls es jemand aus deiner Familie war",
        "sips": 3,
        "all": true,
        "set": "Pflicht"
    },
    {
        "title": "Nicht dein Ernst oder",
        "text": "Verteile Schlücke, wenn du noch nie einen der Wilde Kerle Filme gesehen hast",
        "sips": 3,
        "all": false,
        "set": "Wahrheit"
    },
    {
        "title": "Immer diese alten",
        "text": "Dinge, die man nicht mehr macht, wenn man alt ist. {NAME1} beginnt! Wem als erstes nix mehr einfällt trinkt",
        "sips": 3,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Du Giraffe",
        "text": "Kannst du deine Nase mit der Zunge berühren? Verteile Schlücke",
        "sips": 2,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Phew",
        "text": "Hast du schon einmal einen Flammen-Furz gemacht?",
        "sips": 4,
        "all": true,
        "set": "Wahrheit"
    },
    {
        "title": "So eine also",
        "text": "Jeder, der ein Festival-Bändchen trägt, trinkt für jedes Bändchen.",
        "sips": 2,
        "all": true,
        "set": "Pflicht"
    },
    {
        "title": "Prost darauf",
        "text": "Alle Männer trinken!",
        "sips": 5,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Wet-TShirt-Contest?",
        "text": "Jeder, der ein weißes Oberteil trägt, trinkt",
        "sips": 2,
        "all": true,
        "set": "Wahrheit"
    },
    {
        "title": "Du *$^&#**",
        "text": "Alle Beleidigungen mit Tiernamen. Die Person, die keine Ideen mehr hat, oder ein Wort wiederholt, trinkt. [NAME1] beginnt!",
        "sips": 3,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Abstimmung!",
        "text": "Lieber blind und ohne Arme oder taub und ohne Beine sein? Die Außenseiter trinken.",
        "sips": 2,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Warum nicht färben_",
        "text": "Die Schwarzhaarigen unter euch trinken",
        "sips": 3,
        "all": true,
        "set": "Wahrheit"
    },
    {
        "title": "Bisschen Mathe",
        "text": "Es trinken alle, die ein gerades Alter haben",
        "sips": 1,
        "all": true,
        "set": "Wahrheit"
    },
    {
        "title": "Schnipp Schnapp",
        "text": "Der/diejenige mit den längsten Haaren trinkt.",
        "sips": 3,
        "all": false,
        "set": "Pflicht"
    },
    {
        "title": "Abstimmung!",
        "text": "Lieber Strandurlaub oder Städtetrip? Die Außenseiter trinken",
        "sips": 3,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Die Genießer",
        "text": "Alle, die gerade Wein trinken, trinken.",
        "sips": 5,
        "all": true,
        "set": "Pflicht"
    },
    {
        "title": "Schnipp Schnapp",
        "text": "Der/diejenige mit den wenigsten Haaren trinkt.",
        "sips": 3,
        "all": true,
        "set": "Pflicht"
    },
    {
        "title": "Themenspiel",
        "text": "Begriffe zum Thema 'Deutsche Rapper'. [NAME1] beginnt! Wem als erste nix einfällt trinkt",
        "sips": 2,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Hui",
        "text": "Dreh dich 8 mal im Kreis. Danach trinke.",
        "sips": 5,
        "all": false,
        "set": "Pflicht"
    },
    {
        "title": "Abstimmung!",
        "text": "Lieber blind und ohne Arme oder taub und ohne Beine sein? Die Außenseiter trinken",
        "sips": 4,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Willkommen",
        "text": "Trink wenn du zum ersten mal TrinkTrink spielst",
        "sips": 2,
        "all": true,
        "set": "Wahrheit"
    },
    {
        "title": "Einmal Traube-Minze bitte!",
        "text": "Wenn du schon einmal Shisha geraucht hast, trink!",
        "sips": 3,
        "all": true,
        "set": "Wahrheit"
    },
    {
        "title": "Die können das ja",
        "text": "Alle Dorfkinder trinken",
        "sips": 5,
        "all": true,
        "set": "Pflicht"
    },
    {
        "title": "Erzähl mal",
        "text": "Hattest du schon einmal einen One-Night-Stand? Wenn ja, verteile Schlücke, wenn nicht, dann trink sie selbst",
        "sips": 2,
        "all": false,
        "set": "Wahrheit"
    },
    {
        "title": "Hoch die Dinger!",
        "text": "Alle, deren Glas weniger als halb voll ist, trinken auf ex!",
        "sips": -1,
        "all": true,
        "set": "Pflicht"
    },
    {
        "title": "Orangenbaum blätter",
        "text": "Lieder von Peter Fox. [NAME1] beginnt. Wem keins mehr einfällt trinkt!",
        "sips": 3,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Schwere Entscheidung",
        "text": "Trink oder gib einem Spieler deiner Wahl 1 Euro",
        "sips": 4,
        "all": false,
        "set": "Pflicht"
    },
    {
        "title": "Alles Gute!",
        "text": "Wer im Monat April oder November Geburtstag hat, muss exen",
        "sips": -1,
        "all": true,
        "set": "Wahrheit"
    },
    {
        "title": "MC oder BK",
        "text": "Trinke, wenn du schon einmal einen Salat in einem Fastfood-Restaurant gegessen hast.",
        "sips": 2,
        "all": false,
        "set": "Wahrheit"
    },
    {
        "title": "iDrink",
        "text": "Jeder Apple Nutzer trinkt",
        "sips": 4,
        "all": true,
        "set": "Pflicht"
    },
    {
        "title": "Ehm, ok",
        "text": "Der/die Spieler, der/die den ältesten Sexpartner hatte, verteilt Schlucke",
        "sips": 4,
        "all": false,
        "set": "Minigame"
    },
    {
        "title": "Einmal ein Ranger, immer ein Ranger",
        "text": "Farben der Power Rangers. [NAME1] beginnt. Die Person, die keine Ideen mehr hat, oder eine Farbe wiederholt, trinkt!",
        "sips": 2,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Mobile Gamer",
        "text": "Alle, die schon einmal Mario Run gespielt haben, trinken.",
        "sips": 4,
        "all": true,
        "set": "Wahrheit"
    },
    {
        "title": "Brünett",
        "text": "Die Braunhaarigen unter euch trinken.",
        "sips": 3,
        "all": true,
        "set": "Pflicht"
    },
    {
        "title": "Glatze",
        "text": "Derjenige mit den wenigsten Haaren trinkt.",
        "sips": 2,
        "all": true,
        "set": "Pflicht"
    },
    {
        "title": "Tittenbonus",
        "text": "Alle Frauen trinken!",
        "sips": 3,
        "all": true,
        "set": "Pflicht"
    },
    {
        "title": "Horst, Horst",
        "text": "[NAME1] bekommt für den ganzen Abend den Namen „Horst“. Sobald jemand ihn nicht mit diesem Namen anredet, muss derjenige trinken.",
        "sips": 1,
        "all": true,
        "set": "Virus"
    },
    {
        "title": "Roadtrip",
        "text": "Dinge, die man im Kofferraum eines Autos finden kann! [NAME1] beginnt.",
        "sips": 3,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Ganz einfach",
        "text": "Vorteile ein Mann zu sein! [NAME1] beginnt. Wem als ersten nix mehr einfällt trinnkt.",
        "sips": 3,
        "all": true,
        "set": "Minigame"
    },
    {
        "title": "Einmal waxing?",
        "text": "Trink, wenn dein Rücken behaart ist.",
        "sips": 3,
        "all": false,
        "set": "Wahrheit"
    },
    {
        "title": "Gönner",
        "text": "Bestimme einen Mitspieler. Dieser darf verteilen",
        "sips": 2,
        "all": false,
        "set": "Pflicht"
    },
]
